...
FASTAPI_BACKEND_ENDPOINT = "http://placeholder"
    ...
    with st.sidebar:
        ...
        try:
            ...
            if backend_request.status_code == 200:
                st.success("Backend online :white_check_mark:")
            else:
                st.warning("Problem connecting :sob:")
        except requests.ConnectionError as ce:
            LOGGER.error(ce)
            LOGGER.error("Backend offline :scream:")
            st.error("Backend offline :scream:")

    st.write("# Brain Tumor Classification! :brain:")
    st.write("""
    ## Introduction
